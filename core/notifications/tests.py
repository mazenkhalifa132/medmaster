...
class NotificationSystemTests(TestCase):
    def setUp(self):
        ...
        self.other_student = User.objects.create_user(
            username='other', email='other@example.com', password='StrongPass1'
        )
        self.staff = User.objects.create_superuser(
            username='staff', email='staff@example.com', password='StrongPass1'
        )

    def test_new_learning_content_notifies_every_student(self):
        module = Module.objects.create(year=1, name='Anatomy', icon_class='bx-book')
        ...
        self.assertEqual(Notification.objects.filter(recipient=self.other_student).count(), 6)
    ...
    def test_seen_and_expired_notifications_are_not_returned_to_the_bell(self):
        ...
        response = self.client.post(reverse('notifications-mark-all-seen'))

        self.assertEqual(response.status_code, 200)
        active.refresh_from_db()
        self.assertIsNotNone(active.seen_at)
